...
| Range-based `for` loops (`for i in start..end` / `start..=end`) | Parsed in `parser.parse_for` and desugared to canonical C-style `ForStmt` (`init`, `<`/`<=` condition, `+= 1` step), then validated/lowered by existing semantic/Py/LLVM loop pipelines | `tests/test_parser.py::test_parse_range_for_desugars_to_cstyle_loop`, `tests/test_parser.py::test_parse_range_for_inclusive_uses_lte_condition`, `tests/test_semantic.py::test_range_for_loop_typechecks`, cross-backend runtime in `tests/test_golden_helpers.py::test_range_for_and_match_wildcard_consistent_across_backends` |
| Match wildcard arm (`_`) and Bool-arm coverage rules | Wildcard parsing in `parser.parse_match` (`WildcardPattern`), semantic checks in `semantic._check_stmt` (wildcard-last, duplicate Bool arm rejection, Bool exhaustiveness with wildcard), backend lowering in `codegen._stmt_py` and `llvm_codegen._compile_stmt` | `tests/test_parser.py::test_parse_match_accepts_wildcard_pattern`, `tests/test_semantic.py::test_match_wildcard_makes_bool_match_exhaustive`, `tests/test_semantic.py::test_match_wildcard_must_be_last`, `tests/test_semantic.py::test_match_duplicate_bool_pattern_is_rejected`, runtime in `tests/test_golden_helpers.py::test_range_for_and_match_wildcard_consistent_across_backends` |
...
| Structured diagnostics for `astra check --json` with stable codes (`ASTRA-*`) | CLI handling in `astra/check.py` and `astra/cli.py`; LSP and CLI share the same check pipeline | `tests/test_check.py::*` (JSON and span structure), `tests/test_tooling_inprocess.py::test_lsp_helpers_and_main_dispatch` (parses diagnostics and asserts `ASTRA-TYPE-0001` code) |
