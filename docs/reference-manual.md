...
- CPU multiversioning:
  ...
  - `--cpu-dispatch` emits baseline/sse4/avx2/avx512 variants (subject to `--cpu-target`) and a runtime dispatcher
  - `--cpu-target baseline|avx2|native` controls emitted variant set (`native` enables full set)
